    ...
    <div className="w-full">
      ...
      <ResponsiveContainer width="100%" height={360}>
        <BarChart data={data} margin={{ top: 10, right: 16, bottom: 48, left: 0 }}>
          ...
          <Tooltip />
          <Legend />
          <Bar yAxisId="left" dataKey="orders" name="注文件数" onClick={(_, idx) => onPickMenu?.(data[idx])} />
          <Bar yAxisId="right" dataKey="sales" name="売上金額" onClick={(_, idx) => onPickMenu?.(data[idx])} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
