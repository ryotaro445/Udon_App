    ...
    <div className="p-6 max-w-6xl mx-auto space-y-4 [writing-mode:horizontal-tb]">
        ...
        <div className="flex items-center gap-2">
          {tab === "daily" || tab === "hourly" ? (
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <option value="all">全メニュー合計</option>
      ...
      {tab === "forecast" && (
        <div className="rounded-2xl border bg-white p-4 shadow-sm space-y-3">
          <h2 className="font-semibold">需要予測（上限〜下限）</h2>
          {loading ? (
            <div>読み込み中…</div>
          ) : forecast.length === 0 ? (
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-2 py-1 text-right">需要予測値</th>
                      <th className="px-2 py-1 text-right">下限</th>
                    </tr>
                  </thead>
                  <tbody>
                    {forecast.map((r, i) => (
