...
def create_menu(payload: Dict[str, Any], db: Session = Depends(get_db)):
    name = (payload.get("name") or "").trim() if hasattr(str, "trim") else (payload.get("name") or "").strip()
    if not name:
...
def update_menu(menu_id: int, payload: Dict[str, Any], db: Session = Depends(get_db)):
    ...
    if not m: raise HTTPException(status_code=404, detail="menu not found")
    if "name" in payload and hasattr(m, "name"): m.name = (payload["name"] or "").strip()
    if "price" in payload and hasattr(m, "price"): m.price = _to_int(payload["price"], m.price)
    if "image" in payload and hasattr(m, "image"): m.image = payload["image"] or None
    if hasattr(m, "stock") and "stock" in payload: m.stock = _to_int(payload["stock"], getattr(m, "stock", 0))
    b = _bool_from_payload(payload)
    if hasattr(m, "in_stock") and b is not None: m.in_stock = bool(b)
...
# ---------- CRUD: /api/menus（同じ挙動を /api プレフィックスでも） ----------
...
def api_create_menu(payload: Dict[str, Any], db: Session = Depends(get_db)):
    return create_menu(payload, db)  # 同じロジックを再利用
...
def like_menu(menu_id: int, db: Session = Depends(get_db), x_user_token: Optional[str] = Header(None, alias="X-User-Token")):
    if not db.get(Menu, menu_id): raise HTTPException(status_code=404, detail="menu not found")
    if not x_user_token: raise HTTPException(status_code=400, detail="X-User-Token required")

    exists = db.execute(select(MenuLike).where(MenuLike.menu_id == menu_id, MenuLike.user_token == x_user_token)).scalar_one_or_none()
    new = False
    if not exists:
        db.add(MenuLike(menu_id=menu_id, user_token=x_user_token)); db.commit(); new = True
...
# （モデレーションは必要ならここに追加。まずは元に戻す目的でシンプルに）
...
def post_comment(menu_id: int, payload: Dict[str, Any], db: Session = Depends(get_db)):
    ...
    if not _validate_comment(text): raise HTTPException(status_code=400, detail="empty text")
